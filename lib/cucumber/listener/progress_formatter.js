var ProgressFormatter = function(configuration) {
  ...
  if (!configuration)
    configuration = {};
  if (configuration['logToConsole'] == undefined)
    configuration['logToConsole'] = true;
  var self = {
    log: function log(string) {
      logs += string;
      if (configuration['logToConsole'])
        process.stdout.write(string);
      if (typeof(configuration['logToFunction']) == 'function')
        configuration['logToFunction'](string);
    },
    ...
    storeUndefinedStep: function storeUndefinedStep(step) {
      var snippetBuilder = Cucumber.SupportCode.StepDefinitionSnippetBuilder(step, configuration);
